...
our $DATE =	'$Date: 2017-06-01 23:08:22 -0400 (Thu,  1 Jun 2017) $';  
...
for my $i (0 .. @queue-1) {
	...
	if ($mrnaseq) {
		...
		while (1) {		#sometimes aa1 and aa2 may have identical tails
			if ($pos1 and substr($aa1, length($aa1)-1, 1) eq substr($aa2, length($aa2)-1, 1)) {
				$aa1 =~ s/.$//;
				$aa2 =~ s/.$//;
				$pos2--;
			} else {
...
if ($newevf) {
	...
	while (<EVF>) {
		...
		m/^line\d+/ or die "Error: invalid record found in exonic_variant_function file $evffile (line number expected): <$_>\n";
		my @field = split (/\t/, $_);
		if ($field[2] =~ m/^\w+:(\w+):wholegene/ or $field[2] =~ m/unknown/i) {
			print NEWEVF $_, "\n";
			next;
		}
...
=head1 SYNOPSIS
 ...
 Version: $Date: 2017-06-01 23:08:22 -0400 (Thu,  1 Jun 2017) $
